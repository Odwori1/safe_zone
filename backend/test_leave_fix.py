"""
Test leave room with current RLS policies
"""
import asyncio
...
async def test_leave():
    ...
    try:
        async with database.pool.acquire() as conn:
            ...
            # Check current state
            before = await conn.fetchrow("SELECT * FROM live_audio_room_participants WHERE room_id = $1 AND user_id = $2", room_id, user_id)
            print(f"BEFORE - left_at: {before['left_at']}")
