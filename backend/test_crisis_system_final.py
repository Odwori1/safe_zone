#!/usr/bin/env python3
"""
Test script for Crisis Resources system - FINAL VERSION
...
async def test_crisis_crud():
    """Test CRUD operations for crisis resources - FINAL VERSION"""
    ...
    try:
        ...
        # Test crisis preferences - HANDLE EXISTING PREFERENCES
        print("🔧 Testing Crisis Preferences...")
        from app.schemas.crisis import UserCrisisPreferencesCreate, UserCrisisPreferencesUpdate
        
        # Check if preferences already exist
        existing_preferences = await crisis_crud.get_user_crisis_preferences(user_id)
        
        if existing_preferences:
            # Update existing preferences
            print("   Updating existing crisis preferences...")
            update_prefs_data = UserCrisisPreferencesUpdate(
                emergency_contact_instructions="Updated instructions",
                consent_to_contact=True
            )
            updated_preferences = await crisis_crud.update_user_crisis_preferences(user_id, update_prefs_data)
            if updated_preferences:
                print("✅ Update crisis preferences: PASS")
            else:
                print("❌ Update crisis preferences: FAIL")
                return False
        else:
            # Create new preferences
            print("   Creating new crisis preferences...")
