-- Professional Directory Schema for Phase 3, Item 7 - FIXED VERSION
-- Following EXACT same patterns as enhanced_moderation_schema_fixed.sql

-- Enable btree_gist extension for UUID GiST indexes
...
-- Professional Availability Table (scheduling) - SIMPLIFIED WITHOUT GiST
CREATE TABLE IF NOT EXISTS professional_availability (
    ...
    -- Note: Removed GiST exclusion constraint for simplicity
    -- Application logic will handle overlapping slot prevention
);
...
-- Appointments Table (booking system) - SIMPLIFIED WITHOUT GiST
CREATE TABLE IF NOT EXISTS appointments (
    ...
    -- Note: Removed GiST exclusion constraint for simplicity
    -- Application logic will handle overlapping appointment prevention
);
...
-- Create indexes for performance (basic indexes only)
